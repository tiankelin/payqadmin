  ...
  <div class="main conMain">
    <div class="mainTop">
      <Form inline :label-width="98">
        ...
        <FormItem label="绑瓶开始时间">
          <DatePicker style='width: 170px;' type="datetime" placeholder="绑瓶开始时间" v-model='bindStartTime' format="yyyy-MM-dd HH:mm:ss"
            @on-change='changeBindStartTime'></DatePicker>
        </FormItem>
        <FormItem label="绑瓶结束时间">
          <DatePicker style='width: 170px;' type="datetime" placeholder="绑瓶结束时间" v-model='bindEndTime' format="yyyy-MM-dd HH:mm:ss"
            @on-change='changeBindEndTime'></DatePicker>
        </FormItem>
        <!-- <FormItem label="建档时间">
        ...
        <FormItem label="绑瓶时间">
          <DatePicker v-model="bindTime" type="datetimerange" style="width: 170px" @on-change="bindChange" format="yyyy-MM-dd HH:mm"
            :editable='false'></DatePicker>
        </FormItem> -->

        <FormItem>
          <Button type="info" @click='toOriginalFile' style="margin:0 10px;">原档案</Button>
          <Button type="success" @click="addClick" v-has='786' style="margin:0 10px;">新增</Button>
          <Button type="primary" @click="searchClick" style="margin:0 20px 0 10px;">查询</Button>
          <!-- <Button type="warning" @click="importClick" style="margin:0 10px;">导入</Button> -->
          <Upload :action="fileUrl" style='display: inline-block;' :show-upload-list='false' :on-success='importSuccess'
            ...
            <Button type="warning" v-has='1013'>导入</Button>
          </Upload>
          <!--<Upload :action="fileUrl1" style='display: inline-block;margin-left: 20px;' :show-upload-list='false'
            :on-success='importSuccess1' :headers="{'Authorization':'Bearer_'+token}" v-has='1008'>
            <Button type="warning">导入更新</Button>
          </Upload>-->
          <Button type="warning" v-has='1008' style="margin-left: 20px;" @click="handleImportUpdate">导入更新</Button>
        </FormItem>
      </Form>
    ...
    <div class="mainContent">
      <Table border :columns="columns" :data="dataList" :loading='loading' highlight-row :height='tableHeight'>
        ...
        <template slot-scope="{ row, index }" slot="action">
          ...
          <Button type="success" size="small" style="margin-right: 10px" @click="handleUnbindClick(row.bottleId)" v-show='row.bottleNfcId' v-has='"newCylinder-unBind"'>解绑</Button>
    ...
    <importUpdate v-if='closeUpdate' @closeUpdate='closeUpdateMethod'></importUpdate>
  </div>
</template>
  ...
  import importUpdate from './importUpdate';
	export default {
      ...
      importUpdate
        ...
        fileUrl: pathUrls.bottleBatchImport,
				tableHeight: 'auto',
...
//					},
      ...
      //改变绑瓶结束时间
        changeBindEndTime(v){
        	if(v){
        		...
        		if(ends=='00:00:00'){
        			this.bindEndTime=starts+'23:59:59';
        		}else{
        			this.bindEndTime=v;
        		}
        	}
      ...
      //改变绑瓶起始时间
        changeBindStartTime(v){
            this.bindStartTime=v;
      ...
      handleUnbindClick(id){
          this.$Modal.confirm({
			...
			toOriginalFile(){
				this.$router.push('/cylinderManage/cylinder');
			},

			//新增
			addClick(){
				this.$router.push('/newCylinder/addNewCyl')
			},
			...
			edit(id) {
				this.$router.push('/newCylinder/editNewCyl' + '/' + id)
			},
			remove(id) {
				this.$Modal.confirm({
					title: '是否注销？',
					...
					onOk: () => {
						...
						_http.http2('post', pathUrls.platformDelete,
							JSON.stringify([id])
						).then((res) => {
							if(res.code == 0) {
								this.$Message['success']({
			...
			getMessage() {
				this.loading = true;
				_http.http1("post", pathUrls.platformList, {
					page: this.pageNum,
					limit: this.pageSize,
					...
					bindStartTime:this.bindStartTime?(this.common.conformatDat(this.bindStartTime,true)):'',//绑瓶起始时间
					bindEndTime:this.bindEndTime?(this.common.conformatDat(this.bindEndTime,true)):'',//绑瓶结束时间
...
//						item.newBottleIsNew=item.bottleIsNew?'新钢瓶':'旧钢瓶'
					}
					}
