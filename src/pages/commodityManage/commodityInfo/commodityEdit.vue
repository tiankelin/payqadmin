		...
		methods: {
			...
			getDeptgoodsInfo() {
				_http.http1('get', pathUrls.deptgoodsInfo + '/' + this.$route.params.id, {}, 'form').then((res) => {
					...
					this.typeForm.createTime = res.deptGoods.createTime;
					this.goodsId=res.deptGoods.goodsId;
					this.updateTime=res.deptGoods.updateTime;
					this.typeForm.organizeOwn=res.deptGoods.orgId+'';
					this.creater=res.deptGoods.creater;
					this.typeForm.unitPrice=res.deptGoods.unitPrice;
					this.typeForm.goodsPic=res.deptGoods.goodsPic;
					this.typeForm.goodsType=res.deptGoods.goodsType;
					this.typeForm.goodsSpec=res.deptGoods.spec;
				})
			},
			//点击确定
			enterClick(){
