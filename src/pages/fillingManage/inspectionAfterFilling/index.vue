  ...
  <div class="main conMain">
    <div class='mainTop conMainTop'>
      <Form inline :label-width="100">
       ...
       <FormItem label="所属组织">
          <Cascader :data="options" placeholder="所属组织" style='width: 220px;' clearable change-on-select @on-change='changeCascader' :render-format="format"
            ></Cascader>
        </FormItem>
  ...
  import _http from '@/public/http';
	...
	export default{
        ...
        organize:'',
          ...
          {
          	title: '复称检查',
          	key: 'newWeighingCheck',
          	minWidth: 100,
          	align: 'center'
          },
          {
          	title: '查漏',
          	key: 'newLeakDetection',
          	minWidth: 100,
          	align: 'center'
          },
          {
		...
		getPrefillcheckList() {
				this.loading = true;
				_http.http1("post", pathUrls.postfillcheckList, {
					page: this.curpage,
          ...
          deptId: this.organize,
            ...
            for(let item of res.data){
                ...
                item.newBaseDamaged=item.baseDamaged?'√':'×';
                item.newWarningLabel=item.warningLabel?'√':'×';
                item.newFillCapacity=item.fillCapacity?'√':'×';
                item.newWeighingCheck=item.weighingCheck?'√':'×';
                item.newLeakDetection=item.leakDetection?'√':'×';
                item.newFillLabel=item.fillLabel?'√':'×';
            }
