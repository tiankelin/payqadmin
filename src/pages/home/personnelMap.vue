<template>
  <div class="personnelMain">
    ...
    <div class="searchPerson" :class="[expand?'searchPerson':'expandPerson']">
      ...
      <div>
        <div class="someTimes"><Input type="text" style="width: 80px;" @on-blur='getSearch' v-model='secondTime' /><span>分钟未获取定位判定为离线</span></div>
        <div class="someTimes" style="color: #88900c;"><Input type="text" style="width: 80px;" @on-blur='getSearch'
            v-model='dayNum' /><span>天登录过判定为近日上线</span></div>
        ...
        <CheckboxGroup v-model="onlones" style='display: inline-block;margin-left: 5px;margin-bottom: 5px;' @on-change='checkChange'>
          ...
          <Checkbox :label="2" style="color:#88900c;">近日上线({{offLineNum}})</Checkbox>
          <Checkbox :label="0" style="color: #f00;">离线({{recentlyNum}})</Checkbox>
        </CheckboxGroup>
      </div>
        ...
        <div style="width: 878px;height: 100%;">
          ...
          <div class="contentList">
            <div v-for='(item,index) in staffLabel' :key='index' class='itemList' :class="{checkList:item.staffId==clickId,onOutLineList:item.staffOnDuty==2,               onLineList:item.staffOnDuty==1,offLineList:!item.staffOnDuty,}"
              ...
              <div style="width:40px;height: 30px;" class="itemsSel"><img v-if='item.staffFace' :src="item.staffFace"
                  alt="" style="height:30px;width:30px;display: inline-block;border-radius: 50%;" /></div>
              ...
              <div style="width: 58px;color:#0d37d2;" class="itemsSel" :title='item.personLiableBottleNum'
                @click.stop='getPersonLiableBottle(item.personLiableBottles,2)'>
                {{item.personLiableBottleNum}}
              </div>
              <div style="width: 43px;color: #089e36;" class="itemsSel" :title='item.personLiableFullBottleNum'
                @click.stop='getPersonLiableBottle(item.personLiableFullBottles,1)'>
                {{item.personLiableFullBottleNum}}
              </div>
              <div style="width: 43px;color: #f37f0b;" class="itemsSel" :title='item.personLiableEmptyBottleNum'
                @click.stop='getPersonLiableBottle(item.personLiableEmptyBottles,0)'>
...
<script>
  import AMap from 'AMap';
	...
	import cylinderTag from '@/pages/comComponent/cylinderTag1.vue';
	export default {
		name: 'personnelMap',
		components: {
			personnelLocus,
      ...
      getPersonInfo(id){
        let perInfo=[];
        if(this.$store.state.mapUserInfo){
          perInfo=JSON.parse(this.$store.state.mapUserInfo);
        }
          if(perInfo.indexOf('personnelMap-personInfo')!=-1){
...
//     					  marker.setLabel({
//     					  	content: `${points.staffName}<br/>${points.staffDeptName}`,
////     					  	direction:'center'
//     					  	})
