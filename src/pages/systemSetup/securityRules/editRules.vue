		...
		<div class="mainContent">
			<Form ref="typeForm" :model="typeForm" :rules="typeForm" :label-width="180">
				<FormItem label="客户类型" class='userType star'>
					<Select v-model="typeForm.userType" style="width: 300px;" placeholder="请选择客户类型">
				...
				<FormItem label="名单类型" class='listType'>
					<Select v-model="typeForm.listType" style="width: 300px;" placeholder="请选择名单类型" clearable>
				...
				<FormItem label="安检周期(天)" class='secCircle star'>
					<Input v-model="typeForm.secCircle" placeholder="请输入安检周期" type="number" />
				</FormItem>
				...
				<FormItem>
					<Button type="success" @click='editrulesUser'>用户</Button>
					<Button type="primary" @click='editrulesFuc'>确定</Button>
...
//					console.log(res.data.nameList.length)
					if(res.data.standardList.length) {
					...
					if(res.data.nameList.length) {
						for(let item of res.data.nameList) {
							this.datas.push({
								key: item.userId,
								label: item.userRealName
							})
			...
			//获取详情信息
			getRuleInfo() {
				_http.http1('get', pathUrls.ruleInfo + '/' + this.$route.params.id, {}, 'form').then((res) => {
					let data = res.securityCheckRule
					this.typeForm.listType = data.listType;
					this.typeForm.userType =+data.userType;
					this.typeForm.secCircle = data.checkPeriod;
