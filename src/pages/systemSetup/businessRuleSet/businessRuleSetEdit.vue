        ...
        <FormItem label="数据名称及内容" class='star' v-if='valueType'>
          ...
          <div class="enumValueName enumValueName1" v-if='valueType=="boolean"'>
            ...
            <div style="display: flex;">
              <div v-for='(item,index) in valueNameList' :key='item' class="listItem">
                <div>
                  <Input type='text' style='width:80px;' v-model='item.valueName' :placeholder='item.placeholderValue' />
                </div>
                <div>
...
<script>
  import _http from '@/public/http';
	...
	export default {
		name: 'businessRuleSetEdit',
		data() {
			return {
        scope:0,
        ruleDesc:'',
        businessConfigEntityList:null,
        jsonValueName:'',
        moduleId:'',
        webOptions:[],
				isDisabled: false,
      ...
      getDeptrulesInfo() {
              this.jsonList=[];
              this.valueNameList=[];
      	_http.http1('get', pathUrls.deptrulesInfo + '/' + this.$route.params.id, {}, 'form').then((res) => {
      		if(res.code == 0) {
              this.businessConfigEntityList=res.deptRules.businessConfigEntityList;
              this.ruleName=res.deptRules.ruleName;//规则名称
              this.moduleType=res.deptRules.moduleType;//模块类型
              this.moduleId=res.deptRules.moduleId+'';//模块id
              this.code=res.deptRules.code;//配置编码
              this.valueType=res.deptRules.valueType;//数据类型
              this.ruleDesc=res.deptRules.ruleDesc;//配置描述
              this.scope=res.deptRules.scope;
              ...
              if(this.valueType!='jsonArr'){
                  ...
                  let names=[];
                  let values=[];
                  if(res.deptRules.valueName){
                    names=res.deptRules.valueName.split(',');
                  }
                  if(res.deptRules.value){
                    values=res.deptRules.value.split(',');
                  }
                  names.forEach((item,i)=>{
                      this.valueNameList.push({
                          valueName:item,
                          value:values[i],
                          placeholderValue:(this.valueType=='int'||this.valueType=='double')?item:'内容'+(i+1)
                      })
                  })
                  // console.log(this.valueNameList)
              }else {
                  this.jsonValueName=res.deptRules.valueName;
                  if(res.deptRules.value){
                    let valueList=JSON.parse(res.deptRules.value);
                    valueList.forEach((item,i)=>{
                      item.valueNameList=[];
                      let jsonNames=[];
                      let jsonValues=[];
                        if(item.valueName){
                           jsonNames=item.valueName.split(',');
                        }
                        if(item.value){
                           jsonValues=item.value.split(',');
                        }

                         jsonNames.forEach((items,j)=>{
                            item.valueNameList.push({
                                valueName:items,
                                value:jsonValues[j],
                                placeholderValue:(item.valueType=='int'||item.valueType=='double')?item:'内容'+(j+1)
                            })
                         })
                         this.jsonList.push({
      ...
      jsonValueTypeChange(index,v){
          ...
          if(v=='boolean'){
              this.jsonList[index].valueNameList=[{
                  valueName:'',
                  value:true,
                  placeholderValue:"名称1",

              },{
                  valueName:'',
                  value:false,
                  placeholderValue:"名称2",
      ...
      handleAddValueName(){
        ...
        if(this.valueType!='jsonArr'){
          ...
          this.isDisabled = true;
				_http.http2('post', pathUrls.deptrulesUpdate, fData).then((res) => {
					if(res.code == 0) {
