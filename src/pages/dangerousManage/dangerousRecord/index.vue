  ...
  <div class="main conMain">
    <div class='mainTop'>
      <Form :model="formSearch" inline :label-width="70">
        <FormItem label="所属组织">
          <Cascader :data="options" clearable change-on-select :render-format="format" style="width:170px" @on-change='changeCascader'></Cascader>
        </FormItem>
    ...
    <div class="pb">
      <div class="whole" v-for="item in tableData1" :key="item.terminalCode">
        <div><span class="bt">{{item.title}}</span><span class="bt btName">{{item.name}}</span></div>
        <div><span class="lt"></span><span class="ht">YSP35.5</span><span class="ht">YSP118</span></div>
        <div><span class="lt">上车：</span><span class="nr">{{item.bind15KG}}</span><span class="nr">{{item.bind50KG}}</span></div>
        <div><span class="lt">下车：</span><span class="nr">{{item.unbind15KG}}</span><span class="nr">{{item.unbind50KG}}</span></div>
...
//						item.newStatus = item.bindStatus == 1 ? '上瓶' : '下瓶'
