<template>
	<div :style="{width:tableWidth}" class='autoTbale'>
		<table class="table table-bordered" id='hl-tree-table'>
			<thead>	
				<tr>
					<th v-for="(column,index) in cloneColumns" style='background: #E2EEFF;color: #51B5EA;'>
				...
				<tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
					<td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
						...
						<div v-if="column.type === 'action'">
							<i-button type="info" size="small" @click="RowClick(item,$event,index,column.actions[0].text)" style='margin: 0 5px;' v-has='16'>{{column.actions[0].text}}</i-button>
							<i-button type="error" size="small" @click="RowClick(item,$event,index,column.actions[1].text)" style='margin: 0 5px;' v-has='17'>{{column.actions[1].text}}</i-button>	
						</div>
						<label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                               <i v-html='item.spaceHtml'></i>
                               <i v-if="item.children&&item.children.length>0" class="ivu-icon" :class="{'ivu-icon-ios-add':!item.expanded,'ivu-icon-ios-remove':item.expanded }" style='border: 1px solid #dcdee2;cursor: pointer;'></i>                               
                               <i v-if='!item.children||(item.children&&!item.children.length)' class="ms-tree-space ivu-icon" :class="{'ivu-icon-ios-add':!item.expanded,'ivu-icon-ios-remove':item.expanded }" style='visibility: hidden;'></i>
                               <i v-if='(!item.children||(item.children&&!item.children.length))&&item.type==2' class="ms-tree-space ivu-icon" :class="{'ivu-icon-ios-add':!item.expanded,'ivu-icon-ios-remove':item.expanded }" style='visibility: hidden;margin-left: 0px;'></i>
...
<style scoped>
	...
	.child-tr .ivu-icon-ios-remove{
		margin-left: 20px;
	}
	.child-tr .ivu-icon-ios-add{
		margin-left: 15px;
	}
